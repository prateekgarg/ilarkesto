...
public class TemplateBuilder {
	...
	public ComponentIncludeElement startComponentInclude(String componentTemplatePath) {
		return startContainer(new ComponentIncludeElement(componentTemplatePath.trim()));
	}

	public Template startInlineTemplate(String name) {
		ContainerElement container = containerStack.peek();
		if (!(container instanceof ComponentIncludeElement)) return null;
		ComponentIncludeElement component = (ComponentIncludeElement) container;

		Template template = new Template();
		component.addTemplate(name.trim(), template);
		containerStack.push(template);
		return template;
	}
