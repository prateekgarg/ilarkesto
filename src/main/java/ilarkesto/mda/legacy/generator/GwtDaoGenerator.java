...
public class GwtDaoGenerator extends AClassGenerator {
	...
	protected void writeContent() {
		for (EntityModel entity : entities) {
			...
			ln("    }");
			ln();
			ln("    protected " + type + " update" + name + "(Map<String, String> data) {");
			ln("        String id = data.get(\"id\");");
			...
			for (PropertyModel p : entity.getProperties()) {
				...
				if (pType.equals(DateRange.class.getName())) pType = ilarkesto.core.time.DateRange.class.getName();
				if (pType.equals(DayAndMonth.class.getName())) pType = ilarkesto.core.time.DayAndMonth.class.getName();
				if (p.isReference()) {
		...
		ln();
		ln("    @Override");
		ln("    protected final " + AGwtEntity.class.getName()
				+ " updateLocalEntity(String type, Map<String, String> data) {");
		for (EntityModel entity : entities) {
