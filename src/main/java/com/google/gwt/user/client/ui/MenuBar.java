 ...
 * a {@link com.google.gwt.core.client.Scheduler.ScheduledCommand} or open a cascaded menu bar.
 *
 * <p>
...
public class MenuBar extends Widget implements PopupListener, HasAnimation, HasCloseHandlers<PopupPanel> {
	...
	public MenuItem insertItem(MenuItem item, int beforeIndex) throws IndexOutOfBoundsException {
		// Check the bounds
		if (beforeIndex < 0 || beforeIndex > allItems.size()) { throw new IndexOutOfBoundsException(); }
	...
	/**
	 * Adds a thin line to the {@link MenuBar} to separate sections of {@link MenuItem}s at the specified
	 * index.
	 *
	...
	/**
	 * Moves the menu selection down to the next item. If there is no selection, selects the first item. If
	 * there are no items at all, does nothing.
	 */
	public void moveSelectionDown() {
		if (selectFirstItemIfNoneSelected()) { return; }

		if (vertical) {
			selectNextItem();
		} else {
			if (selectedItem.getSubMenu() != null && !selectedItem.getSubMenu().getItems().isEmpty()
	...
	/**
	 * Moves the menu selection up to the previous item. If there is no selection, selects the first item. If
	 * there are no items at all, does nothing.
	 */
	...
	/**
	 * Enable or disable auto focus when the mouse hovers over the MenuBar. This allows the MenuBar to respond
	 * to keyboard events without the user having to click on it, but it will steal focus from other elements
	 * on the page. Enabled by default.
	 *
	...
	/**
	 * Returns a list containing the <code>MenuItem</code> objects in the menu bar. If there are no items in
	 * the menu bar, then an empty <code>List</code> object will be returned.
	 *
	...
	/**
	 * Returns the <code>MenuItem</code> that is currently selected (highlighted) by the user. If none of the
	 * items in the menu are currently selected, then <code>null</code> will be returned.
	 *
	 * @return the <code>MenuItem</code> that is currently selected, or <code>null</code> if no items are
	 *         currently selected
	 */
	...
	/*
	 * Performs the action associated with the given menu item. If the item has a popup associated with it,
	 * the popup will be shown. If it has a command associated with it, and 'fireCommand' is true, then the
	 * command will be fired. Popups associated with other items will be hidden.
	 *
	 * @param item the item whose popup is to be shown. @param fireCommand <code>true</code> if the item's
	 * command should be fired, <code>false</code> otherwise.
	 */
	...
	void itemOver(MenuItem item, boolean focus) {
		if (item == null) {
			...
			if ((selectedItem != null) && (shownChildMenu == selectedItem.getSubMenu())) { return; }
		}
	...
	/**
	 * Physically add the td element of a {@link MenuItem} or {@link MenuItemSeparator} to this
	 * {@link MenuBar}.
	 *
	 * @param beforeIndex the index where the separator should be inserted
	...
	private MenuItem findItem(Element hItem) {
		for (MenuItem item : items) {
			if (item.getElement().isOrHasChild(hItem)) { return item; }
		}
		return null;
	}
	...
	/*
	 * This method is called when a menu bar is hidden, so that it can hide any child popups that are
	...
	private void openPopup(final MenuItem item) {
		...
		popup.setPopupPositionAndShow(new PopupPanel.PositionCallback() {
			...
			public void setPosition(int offsetWidth, int offsetHeight) {
				...
				// of its base item
				if (LocaleInfo.getCurrentLocale().isRTL() || popupPosition == PopupPosition.LEFT) {
					if (vertical) {
	...
	/**
	 * Selects the first item in the menu if no items are currently selected. Has no effect if there are no
	 * items.
	 *
